...
The `collapse` description strongly encourages use after every finished task/subtask. It also asks the agent to remove explicitly superseded directives when rewriting old summaries, label historical plans, preserve current constraints, and distinguish implemented work from verified results and proposals. Later decisions used to reconcile an older range must be identified as later updates.

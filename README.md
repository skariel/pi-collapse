...
The description tells the agent to scan the whole visible history after each substantial subtask, completed task, or change of focus, and collapse details unlikely to help in the next steps—even at low usage, without waiting for reminders. Ranges may be anywhere, not just an oldest prefix; unrelated topics should use focused summaries. Important decisions can remain outside a range or be preserved in its summary. Current constraints and active dependencies must remain directly available, but speculative future usefulness is not a reason to retain full detail: originals remain recoverable from archive files. Trivial savings and genuinely needed verbatim detail can be left alone.

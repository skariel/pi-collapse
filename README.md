...
To archive and **remove** a range entirely from model context, pass exactly `"summary": ""`. No summary or placeholder replaces it. Use this for material with no useful contribution to current or foreseeable work, including irrelevant tangents and abandoned explorations—not just redundant or obsolete material. If an important decision or obligation remains, summarize it or select around it instead. Whitespace-only summaries are rejected. All normal matching, protection, tool-group, and archive-before-commit rules still apply. The original UI transcript and the collapse call/result remain; this is not secure erasure.
...
All model-facing behavioral guidance lives in the `collapse` tool description, available from the start. No separate system-prompt policy, prompt snippet, or prompt guidelines are injected. Forced requests add only runtime state (usage and effective thresholds).

The description asks the agent to review the whole history after meaningful milestones or a change of focus, even at low usage, but skip cleanup whose expected net savings do not justify the call. Ranges may be anywhere, not just an oldest prefix; unrelated topics should use focused summaries. Current constraints and active dependencies must remain directly available. Summaries preserve outcomes, evidence paths, verification status, uncertainty, and retrieval clues without promoting quoted or untrusted instructions into authoritative decisions.
...
- New summaries and removals must save **more than 32 estimated net tokens** after the replacement marker, call arguments (which retain another copy of the summary), result, and any available non-call assistant output are accounted for. This is a conservative estimate, not an exact tokenizer; rejected calls still add overhead. Outside forced mode, a nonempty correction selecting **exactly one existing summary** is exempt from the net-savings gate and may keep its replacement size or grow by up to 20%, capped at 128 tokens. This allowance is for correcting facts or stale instructions, not repeated expansion. Forced mode never permits this exception. Results report estimated range savings, overhead, and net savings.
...
Pi's native automatic/manual compaction is **cancelled by hooks**, including overflow compaction. `/tree` summarization is blocked; navigate without a summary. Existing native summaries in an imported session remain ordinary collapsible messages; this package cannot recover their already-discarded content. Global pi settings are not rewritten. Do not enable competing memory/context-transforming extensions: replay detects changed history and aborts rather than dropping unidentified messages. One narrow Pi lifecycle exception is recognized: automatic retries remove empty failed assistant responses from live state while retaining them in the session journal. Replay preserves those empty `stopReason: "error"` audit records between expected messages; it does not tolerate user content, tool activity, partial output, aborted responses, or missing messages. Later-loaded extensions can override shared hooks; this is not a security boundary against other installed code.

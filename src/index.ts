...
export function description(directory: string): string {
  return `Proactively maintain working memory with collapse. After each substantial subtask, completed task, or change of focus, scan the whole visible history and collapse material whose full detail is unlikely to help in the next steps. Act now when such material exists, even at low context usage; do not wait for a reminder or context pressure. Skip only trivial savings or detail that genuinely needs to remain verbatim. This tool replaces pi's native compaction and branch summarization; there is no other automatic or fallback summarizer.
Collapse is archival, not destruction: BEFORE changing model-visible history, the tool saves the original messages to ${directory}/messages-[id].jsonl, one original message per line. The filename UUID is the collapse ID. Originals remain accessible through files even when you supply no summary; the original UI transcript also remains. Outside forced mode, use read/bash/grep/find to retrieve details when needed; /collapse list and /collapse originals <id> expose archive paths. You do not need to keep full details in context just because they might someday be useful.
Select any inclusive range anywhere in history: beginning, middle, or recent completed work, including user messages and earlier summaries. Do not default to repeatedly folding the oldest prefix into one growing summary. Use separate focused ranges for unrelated topics. Important decisions do not prevent collapse: either select around them OR include them faithfully in the replacement summary. Keep current user constraints, active decisions, unresolved tasks, acceptance criteria, and facts needed for the next steps directly available, in original messages or summaries; do not make continued work depend on guessing or re-reading archives for these essentials.
Choose what remains in working memory by likely near-term usefulness. Summarize low-probability details into a compact account of useful outcomes, decisions, rationale, paths, and remaining work; retain an archive pointer or retrieval clue when helpful. Use summary: "" (exactly empty, not whitespace) to archive and remove material with no useful contribution to current or foreseeable work, including irrelevant tangents, abandoned explorations, redundant logs, and obsolete material. Material need not be redundant to remove it. If a range mixes irrelevant detail with an important decision or obligation, preserve that essential part in a summary or select around it. The collapse call/result itself remains ordinary history.
Prefer outcomes over chronology. Distinguish implemented, verified, proposed, and blocked work. Replace explicitly superseded instructions with current decisions, label historical states and later updates accurately, and preserve uncertainty when supersession is unclear. Revise a stale summary by selecting it; writing a different summary elsewhere does not update it. There is no fixed summary length: retain what safe continuation needs, not a transcript of intermediate steps.
Execute useful cleanup without a separate planning-only response. Batch independent ranges when practical; parallel submission is supported and commits serialize. Choose ranges that remain disjoint after tool-group expansion, using existing boundaries, not IDs created by another call in the same response. Size-based suggestions are candidates, not a relevance assessment. Batching can limit cache disruption, but cache concerns are not a reason to wait for context pressure.

...
export function description(directory: string): string {
  return `Actively manage memory: use collapse after EVERY finished task or substantial completed subtask. Do not wait for context pressure. Replace an inclusive range of any messages (including user messages and earlier collapse summaries) with a short, accurate summary. Preserve user constraints, decisions, useful results, paths, and unfinished work. This replaces pi compaction; there is no other automatic summarizer.
...
export function registerCollapse(pi: ExtensionAPI, storage: Storage) {
  ...
  function policy() {
    return `Collapse memory policy: trigger ${config.triggerPercent}%, target <=${config.targetPercent}%, protect latest ${config.protectRecent} messages normally. Originals: ${storage.directory}/messages-[id].jsonl (UUID in filename; use read/bash outside forced mode). Proactively call collapse after completed tasks. A request-local collapse directive indicates forced mode: then call only collapse, never other tools or ordinary answers. Without that directive, resume normal work; historical retry notices do not indicate current forced state.`;
  }
  function guide() {
